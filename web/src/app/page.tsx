    ...
    <main className="min-h-screen bg-neutral-900 text-white p-10 font-sans selection:bg-rose-500 selection:text-white">
      <div className="max-w-4xl mx-auto">
        <header className="flex justify-between items-center mb-16 border-b border-neutral-800 pb-6">
          <div>
            <h1 className="text-3xl font-black uppercase tracking-wider text-rose-500">Praescius</h1>
            <span className="text-xs font-bold tracking-[0.2em] text-neutral-400">BY NANDURI LABS</span>
          </div>
          {user && (
        ...
        {!user ? (
          <div className="bg-neutral-800 p-8 border-2 border-black shadow-[4px_4px_0_0_#000]">
            <h2 className="text-xl font-black uppercase mb-6 text-rose-500">Access Portal</h2>
            <div className="flex flex-col gap-4">
              <input type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} className="bg-neutral-900 border border-neutral-700 p-3 text-sm focus:outline-none focus:border-rose-500 transition" />
              <input type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} className="bg-neutral-900 border border-neutral-700 p-3 text-sm focus:outline-none focus:border-rose-500 transition" />
              <div className="flex gap-4 mt-2">
                <button onClick={handleSignIn} disabled={loading} className="flex-1 bg-white text-black font-black p-3 hover:bg-neutral-200 transition uppercase text-sm disabled:opacity-50">Log In</button>
                <button onClick={handleSignUp} disabled={loading} className="flex-1 border-2 border-white text-white font-black p-3 hover:bg-white hover:text-black transition uppercase text-sm disabled:opacity-50">Sign Up</button>
              </div>
              ...
              <div className="flex gap-4">
                <button onClick={handleGoogleSignIn} disabled={loading} className="flex-1 flex items-center justify-center gap-2 bg-[#4285F4] text-white font-black p-3 hover:bg-[#357ae8] transition uppercase text-sm disabled:opacity-50">
                ...
                <button onClick={handleAppleSignIn} disabled={loading} className="flex-1 flex items-center justify-center gap-2 bg-black text-white border border-neutral-700 font-black p-3 hover:bg-neutral-900 transition uppercase text-sm disabled:opacity-50">
              ...
              {message && <p className="text-rose-400 text-sm font-bold mt-2">{message}</p>}
            </div>
          </div>
        ) : (
          <div className="flex flex-col gap-8">
            <div className="bg-neutral-800 p-8 border-2 border-black shadow-[4px_4px_0_0_#000]">
              <h2 className="text-sm font-bold text-neutral-400 uppercase tracking-widest mb-2">Current Tier</h2>
              <div className="text-5xl font-black uppercase tracking-tight text-rose-500">{tier}</div>
              <p className="mt-4 text-sm text-neutral-300">
                Email: {user.email} <br/>
                Status: {tier === 'FREE' ? 'Active (Limited)' : 'Active (Unlocked)'}
              </p>
            </div>

            <div className="bg-neutral-800 p-8 border-2 border-black shadow-[4px_4px_0_0_#000]">
              <h2 className="text-xl font-black uppercase mb-6 text-white">Redeem Beta Access</h2>
              <div className="flex gap-4">
                <input type="text" placeholder="Coupon Code (e.g., BETAPRO)" value={coupon} onChange={(e) => setCoupon(e.target.value)} className="flex-1 bg-neutral-900 border border-neutral-700 p-3 text-sm focus:outline-none focus:border-rose-500 transition uppercase" />
                <button onClick={handleRedeemCoupon} disabled={loading} className="bg-rose-500 text-white font-black px-8 py-3 hover:bg-rose-600 transition uppercase text-sm disabled:opacity-50 shadow-[2px_2px_0_0_#000]">Redeem</button>
              </div>
              {message && <p className="text-rose-400 text-sm font-bold mt-4">{message}</p>}
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
              <div className="bg-neutral-900 p-6 border border-neutral-800">
                <h3 className="font-black text-xl mb-2 text-white">FREE</h3>
              ...
              <div className="bg-neutral-800 p-6 border-2 border-rose-500">
                <h3 className="font-black text-xl mb-2 text-rose-500">PRO</h3>
              ...
              <div className="bg-neutral-800 p-6 border border-yellow-500 relative overflow-hidden">
                <div className="absolute top-4 right-4 bg-yellow-500 text-black text-xs font-black px-2 py-1 uppercase">Whale</div>
                <h3 className="font-black text-xl mb-2 text-yellow-500">ULTRA</h3>
                <ul className="text-sm text-neutral-300 space-y-2 font-mono">
                  <li>- Unlimited Charts</li>
                  <li>- Custom JS Plugin Engine</li>
